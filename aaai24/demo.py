...
def build_demo(config, debug=False):
    ...
    def _wrapper(query, progress=gr.Progress()):
        ...
        responses = ["Loading ...", "Loading ...", "Loading ...", "Loading ...", "Loading ..."]
        answer = "Loading ..."

        yield [answer] + ["<h2> 正在获取文档 </h2>"] + knowledge + responses
    ...
    with gr.Blocks(theme=gr.themes.Base(), css=CSS) as demo:
        with gr.Row():
            with gr.Column(elem_id='left-col'):
                ...
                """)
                with gr.Row():
                    query_box = gr.Textbox(show_label=False, placeholder="请在此输入问题并回车",
                                           container=False)

                answer_box = gr.Textbox(lines=5, placeholder="答案", show_label=False, container=False)

                thank_you_label = gr.Label(value="请评价本次回答")

                #rating_select = gr.Radio(choices=["好", "坏"], label="请评价本次回答", show_label=True, container=True)
                rating_select = gr.Radio(choices=["好", "一般", "坏"], container=False)
                submit_button = gr.Button(value="提交评价", elem_id="submit-btn")
                submit_button.click(submit_handler, inputs=[rating_select], outputs=[thank_you_label])
            ...
            with gr.Column(elem_id='right-col'):
                knowledge_explains = ["knowledge1: paragraph-level rerank web docs plus wiki items",
                                      ...
                                      "knowledge5: paragraph-level rerank web docs&wiki items"]
                with gr.Column(elem_id='right-col'):
                    knowledge_boxes = []
                    for i in range(5):
                        knowledge_boxes.append(gr.Textbox(f"知识 {i + 1}", visible=True, label=knowledge_explains[i]))

                with gr.Column(elem_id='right-col'):
                    responses_boxes = []
                    for i in range(5):
                        responses_boxes.append(gr.Textbox(f"回复 {i + 1}", visible=True,
                                                          label=f"回复{i + 1}(基于知识{i + 1})"))
                    # responses_boxes.append(gr.Textbox(f"无知识回复", visible=True,
                    #                                       label=f"无知识"))
