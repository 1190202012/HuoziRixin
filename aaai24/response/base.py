...
class BaseGenerator:
    def __init__(self, config):
        ...
        if config["ban"]:
            ...
            self.batch_response = lambda queries, language_list, knowledge_list, query_only, history, gpu="gpu0": [None] * len(queries)
            return
    ...
    def batch_response(self, queries, language_list, knowledge_list, query_only, history, gpu="gpu0"):
        return self._batch_response(queries, language_list, knowledge_list, query_only, history, gpu) if len(self.result_dict) == 0 else self._batch_response_use_cache(queries, language_list, knowledge_list, query_only, history, gpu)
    ...
    def _batch_response(self, queries, language_list, knowledge_list, query_only, history, gpu="gpu0"):
        pass

    def _batch_response_use_cache(self, queries, language_list, knowledge_list, query_only, history, gpu="gpu0"):
        ...
        if len(no_cache_queries) > 0:
            self._batch_response(no_cache_queries, no_cache_language_list, no_cache_knowledge_list, query_only, history, gpu)
