...
zh_qa_template = {
    ...
    5: "<|beginofutterance|>系统\n你是由哈尔滨工业大学--自然语言处理研究所进行训练和部署的人工智能（Artificial Intelligence, AI）助手。\n你的名字是“活字”。\n你要为用户提供高质量的自然语言处理服务，旨在实现与用户之间的流畅、自然、可信、可靠和可用的对话。\n你的目标是通过对话回答用户的问题、提供相关信息和建议，并能够执行各种任务，以满足用户的需求和期望。\n你需要努力确保我们的服务能够提供准确、有用和全面的解决方案，以使用户获得最佳的体验和价值<|endofutterance|>\n<|beginofutterance|>用户\n文档：{knowledge}\n请根据以上文档继续下列对话，只需要回答一次：{dialogue}\n<|endofutterance|>\n<|beginofutterance|>助手\n"
}
...
en_qa_template = {
    ...
    6: "<|beginofutterance|>系统\n你是由哈尔滨工业大学--自然语言处理研究所进行训练和部署的人工智能（Artificial Intelligence, AI）助手。\n你的名字是“活字”。\n你要为用户提供高质量的自然语言处理服务，旨在实现与用户之间的流畅、自然、可信、可靠和可用的对话。\n你的目标是通过对话回答用户的问题、提供相关信息和建议，并能够执行各种任务，以满足用户的需求和期望。\n你需要努力确保我们的服务能够提供准确、有用和全面的解决方案，以使用户获得最佳的体验和价值<|endofutterance|>\n<|beginofutterance|>用户\ndoc:{knowledge}\nPlease refer to the doc to continue the dialogue,only answer once:{dialogue}\n<|endofutterance|>\n<|beginofutterance|>助手\n"
}
...
class StandardGenerator(BaseGenerator):
    ...
    def _batch_response(self, query_list, language_list, knowledge_list, query_only, history, gpu="gpu0"):
        prompts = []
        infos = []
        if query_only:
            ...
            for query, language, knowledge in zip(query_list, language_list, knowledge_list):
                ...
                for q, response in history:
                    dialogue = dialogue + '用户\n' + q + '\n' + '助手\n' +response + '\n'
                dialogue = dialogue + '用户\n' + query

                if language == "zh":
                    prompt, fill_info = self.zh_prompt_template.build_prompt({"knowledge": knowledge, "dialogue": dialogue})
                else:
                    prompt, fill_info = self.en_prompt_template.build_prompt({"knowledge": knowledge, "dialogue": dialogue})
